...
import { createDemoPresentation } from "./defaults";
...
const DEMO_SEEDED_KEY = "excalidraw-video-deck:demo-seeded";
const INTRO_DISMISSED_KEY = "excalidraw-video-deck:intro-dismissed";
...
// Seeds a sample deck the first time this browser ever opens the app, so a
// fresh visitor sees a working example instead of an empty dashboard. Gated
// by a one-time flag rather than "no presentations exist" so that a user who
// later deletes every deck they made doesn't have the demo silently reappear.
export function seedDemoPresentationIfFirstRun(): void {
  try {
    if (localStorage.getItem(DEMO_SEEDED_KEY)) return;
    localStorage.setItem(DEMO_SEEDED_KEY, "1");
    if (listPresentations().length > 0) return;
    savePresentation(stripVideoSrcs(createDemoPresentation()));
  } catch { /* storage unavailable; skip seeding */ }
}
