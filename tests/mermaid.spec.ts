...
// (addInitScript re-runs before each one), so tests never see the
// auto-created sample deck real first-time visitors get (see
// seedDemoPresentationIfFirstRun in src/lib/storage.ts). Storage itself is
// still reset explicitly after load so seeded tests never depend on the
// relative order of multiple init scripts.
...
// The 50ms settles after open/close are deliberate, not arbitrary: this
// dialog opens from a Radix DropdownMenuItem, and Radix's outside-pointerdown
// listener (and the menu-to-dialog handoff) each register via their own
// internal setTimeout(0). A test that opens and closes this dialog back to
// back with zero delay can occasionally race that registration — no real
// user operates a menu and a dialog within a single JS tick, so this matches
// realistic interaction speed rather than masking a bug (confirmed by
// fuzzing: 0ms delay flakes, 50ms is reliable across repeated runs).
