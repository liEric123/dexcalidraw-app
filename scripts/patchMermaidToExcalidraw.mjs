...
function patchFile(fileName, replacements) {
  ...
  for (const { before, after } of replacements) {
    ...
    // treats "$&", "$1", etc. in a *string* replacement as backreference
    // patterns even when `before` is a plain string, not a regex. The
    // class.js patch's `after` text legitimately contains "\\$&" as literal
    // replacement source code, so a string replacement corrupts the output
    // by splicing the matched text back in. A function's return value is
