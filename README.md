...
**Live demo:** https://excalidraw-video-deck.vercel.app opens with a sample deck, no install or account needed.
...
- The first time you open the app in a browser, a sample deck is created automatically so there's something to look at right away; deleting it doesn't bring it back.
...
This is v1: local-only, browser storage, no accounts. The [live demo](https://excalidraw-video-deck.vercel.app) is a static hosted copy of the same client-side app. However, a backend and account-based sync are not implemented and are planned next.
